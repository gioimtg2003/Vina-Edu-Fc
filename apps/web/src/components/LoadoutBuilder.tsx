                ...
                {PRODUCTS.map((product) => {
                    ...
                    return (
                        <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            className={`relative w-full text-left rounded-2xl border p-4 sm:p-5 cursor-pointer transition-all duration-300 ${cls.border} ${cls.bg} ${isSelected ? `ring-1 ${cls.ring} shadow-lg` : ""}`}
                    ...
                    <div key={i} className="flex items-center flex-1 last:flex-none min-w-0">
                        <div className="flex flex-col items-center gap-1">
                            <motion.div
                                ...
                                className="w-7 h-7 sm:w-8 sm:h-8 rounded-full border-2 flex items-center justify-center text-xs font-bold shrink-0"
                                style={{ color: done || active ? "#fff" : "#64748b" }}
                            >
                                {done ? <CheckCircle2 className="w-3.5 h-3.5" /> : i + 1}
                            </motion.div>
                            {/* Step label — always visible, tiny on mobile */}
                            <span
                                className={`text-[8px] sm:text-[10px] font-mono whitespace-nowrap mt-0.5 ${active ? "text-cyan-400" : done ? "text-cyan-500/70" : "text-slate-600"
                                    }`}
                            >
                                {label}
                            </span>
                        ...
                        {i < STEPS.length - 1 && (
                            <div className="flex-1 h-px mx-2 sm:mx-3 mb-4 relative shrink">
...
export default function LoadoutBuilder() {
    ...
    return (
        <section
            ...
            className="py-16 md:py-24 relative overflow-hidden bg-slate-950"
        >
            {/* Background glows */}
            <div
            ...
            <div className="max-w-[1232px] mx-auto px-4 sm:px-6 relative z-10">
                {/* Header */}
                <motion.div
                ...
                <motion.div
                    ...
                    className="relative rounded-2xl sm:rounded-3xl border border-slate-700/60 bg-slate-900/70 backdrop-blur-xl p-4 sm:p-6 md:p-10 shadow-[0_0_60px_-20px_rgba(6,182,212,0.15)] overflow-hidden"
                >
                    {/* Decorative corner */}
                    ...
                    {/* Step content with AnimatePresence — uses relative layout so height grows with content */}
